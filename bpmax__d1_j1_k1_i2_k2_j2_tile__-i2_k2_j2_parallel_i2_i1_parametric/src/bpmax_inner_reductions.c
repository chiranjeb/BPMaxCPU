...
void bpmax_inner_reductions(long M, long N, long I1, long J1, float**** FTable_4D, float** S2, int* seq2, float** FTable_2D){
	///Parameter checking
	if (!((M >= 3 && N >= 8 && I1 >= 0 && J1 >= I1 && M >= J1+1))) {
	...
	{
		//Domain
		//{i,j,i2|i+i2==0 && i+j==0 && M>=3 && N>=8 && I1>=0 && J1>=I1 && M>=J1+1 && 0>=i && N+i>=1}
		//{i,j,i2|i2==j && M>=3 && N>=8 && I1>=0 && J1>=I1 && M>=J1+1 && i+j>=4 && N+i>=1 && j>=0 && N>=j+1 && 0>=i}
		//{i,j,i2|i2==j && M>=3 && N>=8 && I1>=0 && J1>=I1 && M>=J1+1 && 0>=i+j-3 && i+j>=1 && N>=j+1 && 0>=i}
		//{i,j,i2|i+j==-1 && M>=3 && N>=8 && I1>=0 && J1>=I1 && M>=J1+1 && 0>=i && i+i2>=1 && N>=i2+1}
		//{i,j,i2|i+j==-1 && M>=3 && N>=8 && I1>=0 && J1>=I1 && M>=J1+1 && N>=i2+1 && i+i2>=1 && 0>=i}
		//{i0,i1,i2|i0+i1>=0 && i2>=i1+1 && M>=3 && N>=8 && I1>=0 && J1>=I1 && M>=J1+1 && N>=i2+1 && i1>=-1 && N>=i1+1 && 0>=i0 && i0+i2>=1}
		//{i0,i1,i2|i0+i1>=0 && i2>=i1+1 && M>=3 && N>=8 && I1>=0 && J1>=I1 && M>=J1+1 && 0>=i0 && i1>=-1 && N>=i1+1 && N>=i2+1 && i0+i2>=1}
		int c1,c2,c3;
		S0((-N+1),(N-1),(N-1));
		S5((-N+2),(N-3),(N-1));
		S6((-N+2),(N-3),(N-1));
