...
void bpmax_outer_reductions(long M, long N, long N_sec, long N_tile, long R, long MR, long NR, long P, long I1, long J1, long K1, int* seq1, float** S1, float**** FTable_A, float**** FTable_B, float****** FTable_C, float*** FTable_Pack_A, float*** FTable_Pack_B, float**** FTable_C_I1_J1){
    ...
    if (!((M >= 1 && N >= 8 && N_sec >= 2 && N_tile >= 4 && R >= 0 && N_tile >= R+1 && MR >= 1 && NR >= 1 && P >= 1 && 0 >= P-128 && I1 >= 0 && J1 >= I1 && M >= J1+1 && K1 >= I1 && J1 >= K1+1))) {
     ...
     //printf("\n*****************************************bpmax_outer_reductions(I1:%d, I2:%ld, K2:%ld)*****************************************\n", I1, J1, K1);  
    ...
    #define S0(i,j,i2,i3,i4,i5) bpmax_outer_south_west(M,N,N_sec,N_tile,(j == 0)?R:0,(j == 0 && i4 == 0)?R:0,MR,NR,I1,J1,K1,j,i4,seq1,S1,FTable_C[I1+1][J1-1][j][i4],FTable_C_I1_J1[j][i4])
    ...
    #define S2(i,j,i2,i3,i4,i5) bpmax_r3_section(M,N,N_sec,N_tile,(j == 0)?R:0,(j == 0 && i2 == 0)?R:0,MR,NR,I1,J1,K1,j,i2,S1,FTable_B[j][i2],FTable_C_I1_J1[j][i2])
    ...
    #define S4(i,j,i2,i3,i4,i5) bpmax_r4_section(M,N,N_sec,N_tile,(j == 0)?R:0,(j == 0 && i2 == 0)?R:0,MR,NR,I1,J1,K1,j,i2,FTable_A[j][i2],S1,FTable_C_I1_J1[j][i2])
