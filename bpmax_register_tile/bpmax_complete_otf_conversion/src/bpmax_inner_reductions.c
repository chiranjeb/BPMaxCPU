...
void bpmax_inner_reductions(long M, long N, long N_sec, long N_tile, long R, long MR, long NR, long P, long I1, long J1, int** seq2_t, float**** S2_A, float**** S2_B, float**** S2_C, float** Packed_Table_A, float** Packed_Table_B, float**** FTable_C){
	...
	#define S0(i,j,k,i3,i4) matrix_max_plus_section(N,N_sec,N_tile,R,MR,NR,-i,i3,j,S2_A[-i][j],Packed_Table_B,FTable_C[-i][i3])
	#define S1(i,j,k,i3,i4) matrix_max_plus_section(N,N_sec,N_tile,R,MR,NR,-i,i3,j,Packed_Table_A,S2_B[j][i3],FTable_C[-i][i3])
	#define S2(i,j,i2,i3,i4) bpmax_inner_reductions_finalize(N,N_sec,N_tile,R,MR,NR,-i,j,seq2_t,S2_C,FTable_C,FTable_C[-i][j])
	#define S3(i,j,k,i3,i4) transform_section_like_B_for_register_tile(N,N_sec,N_tile,R,MR,NR,j,i3,FTable_C[j][i3],Packed_Table_B)
	#define S4(i,j,i2,i3,i4) transform_section_like_A_for_register_tile(N,N_sec,N_tile,R,MR,NR,-i,j,FTable_C[-i][j],Packed_Table_A)
	#define S5(i,j,i2,i3,i4) bpmax_inner_reductions_diagonal_tile(N,N_sec,N_tile,R,MR,NR,-i,j,seq2_t,S2_C[-i][j],FTable_C[-i][j])
	{
		//Domain
