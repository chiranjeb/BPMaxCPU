...
void bpmax_single_strand_diagonal_tile(long M, long N, long N_sec, long N_tile, long MR, long NR, long I2, long J2, int** seq2_t, float** C_I2_J2){
	///Parameter checking
	...
	{
		//Domain
		//{i,j,i2|i2==j && M>=3 && N>=8 && N_sec>=2 && N_tile>=4 && MR>=1 && NR>=1 && I2>=0 && J2>=I2 && N_sec>=J2+1 && 0>=i && N_tile+i>=0 && 0>=i+j-3 && j>=0 && N_tile>=j+1}
		//{i,j,i2|i2==j && N_tile+i>=1 && i+j>=4 && M>=3 && N>=8 && N_sec>=2 && N_tile>=4 && MR>=1 && NR>=1 && I2>=0 && J2>=I2 && N_sec>=J2+1 && 0>=i && N_sec>=I2+1 && j>=1 && N_tile>=j+1 && J2>=0}
