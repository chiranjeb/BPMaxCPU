...
void bpmax_inner_reductions(long M, long N, long N_sec, long N_tile, long R, long MR, long NR, long P, long I1, long J1, int** seq2_t, float**** S2_C, float** Packed_Table_A, float** Packed_Table_B, float**** FTable_C){
    ///Parameter checking
    if (!((M >= 1 && N >= 8 && N_sec >= 2 && N_tile >= 4 && R >= 0 && N_tile >= R+1 && MR >= 1 && NR >= 1 && P >= 1 && 0 >= P-128 && I1 >= 0 && J1 >= I1 && M >= J1+1))) {
     ...
     //printf("\n*****************************************bpmax_inner_reductions(I1:%d, I2:%ld)*****************************************\n", I1, J1);  
	...
	#define S0(i,j,i2,i3,i4) transform_section_like_A_for_register_tile(N,N_sec,N_tile,R,MR,NR,-i,j,FTable_C[-i][j],Packed_Table_A)
	#define S1(i,j,i2,i3,i4) transform_section_like_A_for_register_tile(N,N_sec,N_tile,R,MR,NR,-i,j,S2_C[-i][j],Packed_Table_A)
	#define S2(i,j,i2,i3,i4) bpmax_inner_reductions_diagonal_tile(N,N_sec,N_tile,R,MR,NR,-i,j,seq2_t,S2_C[-i][j],FTable_C[-i][j])
	#define S3(i,j,k,i3,i4) matrix_max_plus_section(N,N_sec,N_tile,R,MR,NR,-i,i3,j,Packed_Table_A,Packed_Table_B,FTable_C[-i][i3])
	#define S4(i,j,k,i3,i4) matrix_max_plus_section(N,N_sec,N_tile,R,MR,NR,-i,i3,j,Packed_Table_A,Packed_Table_B,FTable_C[-i][i3])
	#define S5(i,j,k,i3,i4) transform_section_like_B_for_register_tile(N,N_sec,N_tile,R,MR,NR,j,i3,FTable_C[j][i3],Packed_Table_B)
	#define S6(i,j,i2,i3,i4) bpmax_inner_reductions_finalize(N,N_sec,N_tile,R,MR,NR,-i,j,seq2_t,S2_C,FTable_C,FTable_C[-i][j])
	#define S7(i,j,k,i3,i4) transform_section_like_B_for_register_tile(N,N_sec,N_tile,R,MR,NR,j,i3,S2_C[j][i3],Packed_Table_B)
	{
		//Domain
		...
		//{i,j,i2,i3,i4|i4==1 && i3==j && i2==0 && 0>=i && i+j>=1 && N_sec>=j+1 && M>=1 && N>=8 && N_sec>=2 && N_tile>=4 && R>=0 && N_tile>=R+1 && MR>=1 && NR>=1 && P>=1 && 0>=P-128 && I1>=0 && J1>=I1 && M>=J1+1}
		...
		//{i,j,k,i3,i4|i4==1 && k==5 && 0>=i && i+i3>=1 && N_sec>=i3+1 && i+j>=1 && i3>=j+1 && M>=1 && N>=8 && N_sec>=2 && N_tile>=4 && R>=0 && N_tile>=R+1 && MR>=1 && NR>=1 && P>=1 && 0>=P-128 && I1>=0 && J1>=I1 && M>=J1+1}
		...
		int c1,c2,c4;
		S2((-N_sec+1),(N_sec-1),(0),(N_sec-1),(0));
		S2((-N_sec+2),(N_sec-2),(0),(N_sec-2),(0));
		S6((-N_sec+2),(N_sec-1),(0),(N_sec-1),(0));
		S1((-N_sec+2),(N_sec-1),(0),(N_sec-1),(1));
		S0((-N_sec+2),(N_sec-1),(4),(N_sec-1),(0));
		for(c1=-N_sec+3;c1 <= 0;c1+=1)
		 {
		 	...
		 	S6((c1),(N_sec-1),(0),(N_sec-1),(0));
		 	S1((c1),(N_sec-1),(0),(N_sec-1),(1));
		 	S0((c1),(N_sec-1),(4),(N_sec-1),(0));
		 }
#if OUTER_REDUCTIONS_TIMING
    gettimeofday(&time, NULL);
    elapsed_time = (((double) time.tv_sec) + ((double) time.tv_usec)/1000000) - elapsed_time;
