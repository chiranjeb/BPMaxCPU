...
void ospsqmax_float(long M, long N, int* seq1, int* seq2, float**** FTable){
	...
	{
		//Domain
		//{i1,j1,i2,j2,i4,i5|i4==j2 && i2==j1 && i1==0 && M>=2 && N>=2 && j2>=0 && N>=j2+1 && N>=i5+1 && j1>=0 && M>=j1+1 && i5>=j2}
		//{i1,j1,i2,j2,i4,i5|i5==j2 && i4==j2 && i2==i1+j1 && i1>=1 && M>=2 && N>=2 && j1>=0 && M>=j1+1 && j2>=0 && N>=j2+1 && M>=i1+j1+1}
		//{i1,j1,i2,j2,i4,i5|i4==j2 && i2==i1+j1 && i1>=1 && i5>=j2+1 && M>=2 && N>=2 && j1>=0 && j2>=0 && N>=i5+1 && M>=i1+j1+1}
		...
		for(c1=1;c1 <= M-1;c1+=1)
		 {
		 	#pragma omp parallel for private(c3,c4,c5,c6)
