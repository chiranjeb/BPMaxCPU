...
void bpmax_final_reductions(long, long, long, long, long, long, long, float****, int*, int*, float**, float**, float**);
void bpmax_single_strand_s1(long, float*, float**);
void bpmax_single_strand_s2(long, float*, float**);
...
void bpmax(long M, long N, long T1, long T2, long T3, int* seq1, int* seq2, float**** FTable){
	...
	float* _lin_FTable_inner_reductions = (float*)malloc(sizeof(float)*((M-1) * (M) * (N) * (N)));
	mallocCheck(_lin_FTable_inner_reductions, ((M-1) * (M) * (N) * (N)), float);
	float**** FTable_inner_reductions = (float****)malloc(sizeof(float***)*(M-1));
	mallocCheck(FTable_inner_reductions, (M-1), float***);
	for (mz1=0;mz1 < M-1; mz1++) {
	...
	#define S_1(i1,j1,i2,i3,i4) bpmax_final_reductions(M,N,i2,j1+i2,T1,T2,T3,FTable,seq1,seq2,S1[0],S2[0],FTable_inner_reductions[i2][j1+i2])
	#define S_2(i,i1,i2,i3,i4) bpmax_single_strand_s1(M,seq1,S1[i])
	#define S3(i,i1,i2,i3,i4) bpmax_single_strand_s2(N,seq2,S2[i])
	#define S4(i1,j1,i2,i3,i4) bpmax_outer_reductions(M,N,i2,j1+i2,T1,T2,T3,FTable,S1[0],FTable_outer_reductions[i2][j1+i2])
	{
		//Domain
		//{i1,j1,i2,j2,i4|M+i1==0 && M>=3 && N>=3 && T1>=1 && T2>=1 && T3>=1 && i2>=0 && j1>=0 && M>=j1+i2+1 && j2>=0 && i4>=j2 && N>=i4+1}
		//{i1,j1,i2,i3,i4|i4==0 && i3==1 && i1==1 && i2>=0 && j1>=0 && M>=j1+i2+1 && M>=3 && N>=3 && T1>=1 && T2>=1 && T3>=1}
		//{i,i1,i2,i3,i4|i4==0 && i3==0 && i2==0 && i1==0 && i==0 && M>=3 && N>=3 && T1>=1 && T2>=1 && T3>=1}
		//{i,i1,i2,i3,i4|i4==0 && i3==0 && i2==0 && i1==0 && i==0 && M>=3 && N>=3 && T1>=1 && T2>=1 && T3>=1}
		//{i1,j1,i2,i3,i4|i4==0 && i3==0 && i1==1 && i2>=0 && M>=j1+i2+1 && j1>=1 && M>=3 && N>=3 && T1>=1 && T2>=1 && T3>=1}
		int c2,c3,c4,c5;
