...
//function prototypes
void bpmax(long, long, long, long, long, int*, int*, float****);
...
int main(int argc, char** argv) {
	...
	if (argc <= 5) {
		...
		printf("Expecting M,N,ts1,ts2,ts3\n");
		exit(0);
	}
	...
	//Initialisation of ts1
	errno = 0;
	end = 0;
	val = argv[3];
	long ts1 = strtol(val,&end,10);
	if ((errno == ERANGE && (ts1 == LONG_MAX || ts1 == LONG_MIN)) || (errno != 0 && ts1 == 0)) {
		perror("strtol");
		exit(EXIT_FAILURE);
	}
	if (end == val) {
		fprintf(stderr, "No digits were found for ts1\n");
		exit(EXIT_FAILURE);
	}
	if (*end != '\0'){
		printf("For parameter ts1: Converted part: %ld, non-convertible part: %s\n", ts1, end);
		exit(EXIT_FAILURE);
	}
	
	//Initialisation of ts2
	errno = 0;
	end = 0;
	val = argv[4];
	long ts2 = strtol(val,&end,10);
	if ((errno == ERANGE && (ts2 == LONG_MAX || ts2 == LONG_MIN)) || (errno != 0 && ts2 == 0)) {
		perror("strtol");
		exit(EXIT_FAILURE);
	}
	if (end == val) {
		fprintf(stderr, "No digits were found for ts2\n");
		exit(EXIT_FAILURE);
	}
	if (*end != '\0'){
		printf("For parameter ts2: Converted part: %ld, non-convertible part: %s\n", ts2, end);
		exit(EXIT_FAILURE);
	}
	
	//Initialisation of ts3
	errno = 0;
	end = 0;
	val = argv[5];
	long ts3 = strtol(val,&end,10);
	if ((errno == ERANGE && (ts3 == LONG_MAX || ts3 == LONG_MIN)) || (errno != 0 && ts3 == 0)) {
		perror("strtol");
		exit(EXIT_FAILURE);
	}
	if (end == val) {
		fprintf(stderr, "No digits were found for ts3\n");
		exit(EXIT_FAILURE);
	}
	if (*end != '\0'){
		printf("For parameter ts3: Converted part: %ld, non-convertible part: %s\n", ts3, end);
		exit(EXIT_FAILURE);
	}
	...
	bpmax(M, N, ts1, ts2, ts3, seq1, seq2, FTable);
