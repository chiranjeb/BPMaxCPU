...
//function prototypes
void bpmax(long, long, long, long, long, int*, int*, double****);
void bpmax_verify(long, long, long, long, long, int*, int*, double****);
...
int main(int argc, char** argv) {
	...
	//Initialisation of T1
	errno = 0;
	end = 0;
	val = argv[3];
	long T1 = strtol(val,&end,10);
	if ((errno == ERANGE && (T1 == LONG_MAX || T1 == LONG_MIN)) || (errno != 0 && T1 == 0)) {
		perror("strtol");
		exit(EXIT_FAILURE);
	}
	if (end == val) {
		fprintf(stderr, "No digits were found for T1\n");
		exit(EXIT_FAILURE);
	}
	if (*end != '\0'){
		printf("For parameter T1: Converted part: %ld, non-convertible part: %s\n", T1, end);
		exit(EXIT_FAILURE);
	}
	
	//Initialisation of T2
	errno = 0;
	end = 0;
	val = argv[4];
	long T2 = strtol(val,&end,10);
	if ((errno == ERANGE && (T2 == LONG_MAX || T2 == LONG_MIN)) || (errno != 0 && T2 == 0)) {
		perror("strtol");
		exit(EXIT_FAILURE);
	}
	if (end == val) {
		fprintf(stderr, "No digits were found for T2\n");
		exit(EXIT_FAILURE);
	}
	if (*end != '\0'){
		printf("For parameter T2: Converted part: %ld, non-convertible part: %s\n", T2, end);
		exit(EXIT_FAILURE);
	}
	
	//Initialisation of T3
	errno = 0;
	end = 0;
	val = argv[5];
	long T3 = strtol(val,&end,10);
	if ((errno == ERANGE && (T3 == LONG_MAX || T3 == LONG_MIN)) || (errno != 0 && T3 == 0)) {
		perror("strtol");
		exit(EXIT_FAILURE);
	}
	if (end == val) {
		fprintf(stderr, "No digits were found for T3\n");
		exit(EXIT_FAILURE);
	}
	if (*end != '\0'){
		printf("For parameter T3: Converted part: %ld, non-convertible part: %s\n", T3, end);
		exit(EXIT_FAILURE);
	}
	...
	bpmax(M, N, T1, T2, T3, seq1, seq2, FTable);
	...
	//Verification Run
			...
			elapsed_time = (((double) time.tv_sec) + ((double) time.tv_usec)/1000000);
		#endif
    	bpmax_verify(M, N, T1, T2, T3, seq1, seq2, FTable_verify);
    	#ifdef TIMING
    		gettimeofday(&time, NULL);
			elapsed_time = (((double) time.tv_sec) + ((double) time.tv_usec)/1000000) - elapsed_time;
                                        ...
                                        for(c3=0;c3 <= N-1;c3+=1)
                                         {
                                                for(c4=0;c4 <= N-1;c4+=1)
                                                 {
                                                        if( c4 >= c3 ) printf("%10e ", var_FTable(c1,c2,c3,c4));
                                                        else printf("%10e ", 0.0);  
                                                 }
                                                 printf("\n");
                                         }
                                        ...
                                        for(c3=0;c3 <= N-1;c3+=1)
                                         {
                                                for(c4=0;c4 <= N-1;c4+=1)
                                                 {
                                                        if( c4 >= c3 ) printf("%10e ", var_FTable_verify(c1,c2,c3,c4));
                                                        else printf("%10e ", 0.0);
                                                 }
                                                 printf("\n");
                                         }
